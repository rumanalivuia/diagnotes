    ...
    return (
      <div className="content-grid">
        <div className="results-col">
          <div className="results-head"><h2>Recently copied</h2><span className="hint">tap to copy again</span></div>
          {recent.length === 0 ? (
            <div className="empty-state"><div className="big">📋</div><h3>Nothing copied yet</h3><p>Comments you copy land here for fast re-access.</p></div>
          ) : (
            <div className="recent-list">
              {recent.map((r) => {
                const c = visible.find((x) => x.id === r.id) || { title: r.title };
                return (
                  <button key={r.id} className="recent-item" onClick={() => {
    ...
    <div className="content-grid">
      <div className="results-col">
        <div className="results-head">
          ...
          <span className="hint">{visible.length} result{visible.length === 1 ? '' : 's'}</span>
        </div>
