    ...
    <aside className="rail">
      ...
      <nav className="rail-nav" aria-label="Views">
        ...
        {!isPublic && (
          <>
            <button
              className={`nav-btn ${view === 'recent' ? 'active' : ''}`}
        ...
        <div className="rail-section">
          ...
          <div className="filter-list">
            {[
              ...
              <button
                key={o.t}
                className={`filter-row ${filters.type === o.t ? 'active' : ''}`}
                onClick={() => setType(o.t)}
              >
      ...
      <div className="rail-section">
        ...
        <div className="filter-list">
          <button
            className={`filter-row ${!filters.categoryId ? 'active' : ''}`}
            onClick={() => setCat(null)}
          >
            <span className="dot all" />
            All categories
          ...
          {categories.map((c) => (
            <button
              key={c.id}
              className={`filter-row ${filters.categoryId === c.id ? 'active' : ''}`}
              onClick={() => setCat(c.id)}
            >
      ...
      <div className="rail-section">
        ...
        <div className="filter-list">
          <button
            className={`filter-row ${!filters.tag ? 'active' : ''}`}
            onClick={() => setTag(null)}
          >
            <span className="dot all" />
            All tags
          ...
          {tagCloud.slice(0, 12).map((t) => (
            <button
              key={t.name}
              className={`filter-row ${filters.tag === t.name ? 'active' : ''}`}
      ...
      <div className="rail-footer">
        {isPublic ? (
          <>
            <button className="btn btn-primary btn-block" onClick={showLoginPrompt}>
