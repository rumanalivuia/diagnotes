...
export default function AdminView({ onEdit }) {
  ...
  const rejectModalRef = useModalTrap(!!rejecting, () => {
    setRejecting(null);
    setReason('');
  });
  ...
  async function toggleArchive(cat) {
    try {
      await updateCategory(cat.id, { archived: cat.archived ? 0 : 1 });
    } catch {
  ...
  async function doReject() {
    if (!rejecting) return;
    try {
      await rejectShared(rejecting.id, reason.trim() || 'No reason given');
    } catch {
  ...
  return (
    <div>
      <div className="results-head">
        <h2>Admin</h2>
        <span className="hint">category management · review queue</span>
      </div>

      <div className="admin-grid">
        <div className="stat-card">
          <div className="num">{counts.shared}</div>
          <div className="lbl">Shared entries</div>
        </div>
        <div className="stat-card">
        ...
        <div className="stat-card">
          <div className="num">{comments.filter((c) => c.type === 'personal').length}</div>
          <div className="lbl">Personal snippets</div>
        </div>
      </div>
      ...
      <div className="panel">
        ...
        {pending.length === 0 ? (
          <p style={{ color: 'var(--ink-soft)', fontSize: 13.5, margin: 0 }}>
            Nothing awaiting approval. Submissions from staff appear here.
          </p>
        ) : (
          pending.map((c) => (
            <CommentCard
      ...
      <div className="panel">
        ...
        <form className="row-inline" onSubmit={addCat} style={{ marginBottom: 10 }}>
          <input
            className="input"
            style={{ flex: 1 }}
            placeholder="New category name (e.g. Immunology)"
            value={newCat}
        ...
        {categories.map((c) => (
          <div key={c.id} className="cat-row">
            ...
            <button className="mini-btn" onClick={() => toggleArchive(c)}>
          ...
          >
            ...
            <div className="field">
              ...
              <textarea
                ...
                placeholder="e.g. Duplicate of existing entry"
              />
            </div>
