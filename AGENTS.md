...
- **Multi-user model (2026-09-05)**: open signup, shared library for all members, no roles. `findOrProvisionNeonAccount` (`api.js`) auto-creates `accounts` rows for verified Neon principals (`neon_sub` unique, empty `password_hash`/`salt` so HMAC login can never accept them). Lookup order: `email` → `neon_sub` → provision.
...
- `vercel.json:5` rewrites `/neonauth/(.*)→/api` (same-origin Neon Auth proxy), `/api/(.*)→/api` and `/(.*)→/index.html`, `outputDirectory: web/dist`, `functions.api/index.js.includeFiles: server/**` — `api/index.js:1` imports `../server/lib/db.js` + `buildApi`, caches pool, disables `bodyParser`.
...
- **vercel.json BOM breaks CLI**: PowerShell `Set-Content -Encoding utf8` adds a BOM; `vercel --prod --yes` fails with "Couldn't parse JSON file". Use `[System.IO.File]::WriteAllText(path, content, [System.Text.UTF8Encoding]::new($false))` or `node -e` to write without BOM.
...
- **Neon Auth web setup**: web uses `createAuthClient` from `@neondatabase/neon-js/auth` (`web/src/lib/neonAuth.js`); API calls carry short JWTs via `authClient.token()` with `credentials:'include'` (auth is cross-origin). JWTs live ~15 min — `sync.js`/`serverCall` refresh once on 401 then fail. **New origins must be allowlisted** or signup/signin fail with `INVALID_ORIGIN`: `neon neon-auth domain add <url> --project-id … --branch … --org-id …` (+ `domain allow-localhost enable` for dev). Without `VITE_NEON_AUTH_URL`, web falls back to legacy HMAC login.
- **Same-origin Neon Auth proxy** (`api/index.js:12 handleNeonAuthProxy`): the session cookie from Neon Auth is `SameSite=None; Partitioned` — blocked in Safari/Firefox. To avoid third-party cookie restrictions, `vercel.json` rewrites `/neonauth/(.*)→/api` and the serverless function proxies to the real Neon Auth URL. The client bundle uses `VITE_NEON_AUTH_URL=/neonauth` (relative, same-origin) in production, so cookies are first-party. The proxy **must forward `Origin` and `Referer` headers** (Better Auth CSRF check) — `api/index.js:29-30`. For local dev, `VITE_NEON_AUTH_URL` stays the absolute URL with `credentials: 'include'`.
- **Bind-then-verify a Neon Auth change end-to-end**: sign up via `POST {NEON_AUTH_BASE_URL}/sign-up/email` (needs `Origin` header), fetch JWT from `/token` with the session cookie, then hit the app API — this exercises signup → provision → auth without a browser.
